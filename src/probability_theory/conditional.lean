...
begin
  split,
    intros hind hcma, 
    haveI := hcma,
    rw [cond_def, (indep_set_iff_measure_inter_eq_mul hcma.meas hmb.meas μ).mp hind,
  ...
  { have hcond := hcondi hcma,
    refine (indep_set_iff_measure_inter_eq_mul hcma.meas hmb.meas μ).mpr _,
    rwa [ ennreal.inv_mul_eq_iff_eq_mul hcma.meas_nz (measure_ne_top _ a), set.inter_comm,
